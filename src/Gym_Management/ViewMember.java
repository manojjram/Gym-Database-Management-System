...
public class ViewMember extends JFrame implements ActionListener {
		...
		try { 
			  ...
			  while(rs.next()) {
				  ...
				  table[i][j++]=rs.getString("Tcat");
                                  //System.out.println(rs.getString("Tcat"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == b2) {
			try {
				ConnectionClass obj = new ConnectionClass();
				String q = "delete from member where MemID='" + MID + "'";
		...
		if (e.getSource() == b1) {
			new UpdateMember(1).setVisible(true);
			setVisible(false);
		}
		if (e.getSource() == b3) {
